...
const testFuncs = [
	...
	// Class endpoints (from attachments)
	{
		...
		category: "Class",
		method: "GET",
		testedWorks: "Only if class started",
	},
	{
		name: "Get Class Active",
		func: getClassActive,
		hasArgs: true,
		category: "Class",
		method: "GET",
	...
	{
		name: "Get Class Permissions",
		func: getClassPermissions,
		hasArgs: true,
		category: "Class",
		method: "GET",
	},
	...
	{
		name: "Get Class Current Poll",
		func: getClassPollCurrent,
		hasArgs: true,
		category: "Class - Polls",
		method: "GET",
		testedWorks: true,
	...
	{
		name: "Clear Class Poll",
		func: clearClassPolls,
		hasArgs: true,
		category: "Class - Polls",
		method: "POST",
		testedWorks: true,
	...
	{
		name: "Create Class Poll",
		func: createClassPoll,
		hasArgs: true,
		category: "Class - Polls",
		method: "POST",
	},
	...
	{
		name: "Response to Poll",
		func: respondClassPoll,
		hasArgs: true,
		category: "Class - Polls",
		method: "POST",
	},
	...
	{
		name: "Transfer Digipogs",
		func: transferDigipogs,
		hasArgs: true,
		category: "Digipogs",
		method: "POST",
	},
		...
		<div style={{ padding: "0 20px" }}>
			...
			>
				...
				>
					...
					<p>
						Working:{" "}
						{testFuncs.filter((t) => t.testedWorks === true)
							.length - 1}
						/{testFuncs.length - 1}
					</p>
				</Flex>
				<Input
					...
					placeholder="Argument (id, email, or 'email|perm')"
				/>
				<Input
					value={bodyValue}
					onChange={(e) => setBodyValue(e.target.value)}
					placeholder="Body (for POST/PUT requests)"
				/>
				{
					// group tests by category
					Object.entries(
						testFuncs.reduce<Record<string, any[]>>((acc, t) => {
...
async function getUser(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getUserClass(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/class`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getUserClasses(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/classes`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function changePerm(inputValue: string) {
	if (!inputValue)
		return console.warn('changePerm requires input like "email|perm"');
	...
	if (inputValue.includes("|")) {
		...
		try {
			...
			body = { perm: inputValue };
		}
	}

	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(email)}/perm`,
			getFetchOptions("PATCH", body),
		);
		const data = await res.json();
...
async function banUser(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/ban`,
...
async function unbanUser(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/unban`,
			getFetchOptions("PATCH"),
		);
		const data = await res.json();
...
async function verifyUser(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/verify`,
			getFetchOptions("PATCH"),
		);
		const data = await res.json();
...
async function regenerateApiKey(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/api/regenerate`,
...
async function updatePin(inputValue: string) {
	if (!inputValue)
		return console.warn('updatePin requires "id|newPin|oldPin(optional)"');
	let id = "";
	let body: any = {};
	if (inputValue.includes("|")) {
		const [userId, newPin, oldPin] = inputValue
			.split("|")
			.map((s) => s.trim());
		id = userId;
		...
		try {
			...
			body = {
				pin: parsed.pin,
				...(parsed.oldPin ? { oldPin: parsed.oldPin } : {}),
			};
		} catch {
			return console.warn(
				'updatePin requires "id|newPin|oldPin(optional)" or JSON',
			);
		}
	}
	if (!id || !body.pin)
		return console.warn("updatePin requires both id and pin");

	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(id)}/pin`,
			getFetchOptions("PATCH", body),
		);
		const data = await res.json();
...
async function requestPinReset(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/pin/reset`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function resetPinWithToken(inputValue: string) {
	if (!inputValue)
		return console.warn("resetPinWithToken requires JSON body");
	let body: any;
	try {
		body = JSON.parse(inputValue);
	} catch {
		return console.warn(
			'resetPinWithToken requires JSON body like {"pin":"1234","token":"..."}',
		);
	}

	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/pin/reset`,
			getFetchOptions("PATCH", body),
		);
		const data = await res.json();
...
async function getClass(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getClassActive(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/active`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getClassBanned(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/banned`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getClassLinks(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/links`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getClassPermissions(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/permissions`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getClassStudents(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/students`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function createClass(inputValue: string) {
	if (!inputValue)
		return console.warn("createClass requires a body (JSON or class name)");
	let body: any;
	try {
		body = JSON.parse(inputValue);
	} catch {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/create`,
...
async function endClass(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/end`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function joinClass(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/join`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function leaveClass(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/leave`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function startClass(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/start`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function getClassPolls(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/polls`,
			getFetchOptions(),
		);
		const data = await res.json();
		console.log("Get Class Polls:", data);
	} catch (err) {
		console.error("Error getting class polls:", err);
	}
}
...
async function getClassPollCurrent(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/polls/current`,
			getFetchOptions(),
		);
		const data = await res.json();
		console.log("Get Class Current Poll:", data);
	} catch (err) {
		console.error("Error getting current poll:", err);
	}
}
...
async function clearClassPolls(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/polls/clear`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function createClassPoll(inputValue: string, bodyValue: string) {
	if (!inputValue)
		return console.warn("createClassPoll requires body or name");
	let body: any;
	try {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/polls/create`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
...
async function endClassPoll(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/polls/end`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function respondClassPoll(inputValue: string, bodyValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/polls/response`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
...
async function endOwnBreak(inputValue: string, bodyValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/break/end`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
		console.log("End Own Break:", data);
	} catch (err) {
		console.error("Error ending own break:", err);
	}
}
...
async function requestBreak(inputValue: string, bodyValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(inputValue)}/break/request`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
		console.log("Request Break:", data);
	} catch (err) {
		console.error("Error requesting break:", err);
	}
}

async function approveBreak(inputValue: string) {
	if (!inputValue)
		return console.warn('approveBreak requires "classId|userId"');
	const [classId, userId] = inputValue.split("|").map((s) => s.trim());
	if (!classId || !userId)
		return console.warn('approveBreak requires "classId|userId"');
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(classId)}/students/${encodeURIComponent(userId)}/break/approve`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
		console.log("Approve Break:", data);
	} catch (err) {
		console.error("Error approving break:", err);
	}
}
...
async function denyBreak(inputValue: string) {
	...
	const [classId, userId] = inputValue.split("|").map((s) => s.trim());
	if (!classId || !userId)
		return console.warn('denyBreak requires "classId|userId"');
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(classId)}/students/${encodeURIComponent(userId)}/break/deny`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
		console.log("Deny Break:", data);
	} catch (err) {
		console.error("Error denying break:", err);
	}
}
...
async function deleteHelpRequest(inputValue: string) {
	if (!inputValue)
		return console.warn('deleteHelpRequest requires "classId|userId"');
	const [classId, userId] = inputValue.split("|").map((s) => s.trim());
	if (!classId || !userId)
		return console.warn('deleteHelpRequest requires "classId|userId"');
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(classId)}/students/${encodeURIComponent(userId)}/help`,
			getFetchOptions("DELETE"),
		);
		const data = await res.json();
...
async function requestClassHelp(inputValue: string) {
	if (!inputValue)
		return console.warn("requestClassHelp requires an id or body");
	let body: any;
	try {
		body = JSON.parse(inputValue);
	} catch {
		body = { classId: inputValue };
	}
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/class/${encodeURIComponent(body.classId || body.id || "")}/help/request`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
...
async function leaveRoom(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/room/${encodeURIComponent(inputValue)}/leave`,
			getFetchOptions("DELETE"),
		);
		const data = await res.json();
...
async function getRoomTags() {
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/room/tags`,
			getFetchOptions(),
		);
		const data = await res.json();
		console.log("Get Room Tags:", data);
	} catch (err) {
		console.error("Error getting room tags:", err);
	}
}
...
async function joinRoomByCode(inputValue: string) {
	if (!inputValue)
		return console.warn(
			"joinRoomByCode requires a code or JSON with {code}",
		);
	let code = inputValue;
	try {
		const parsed = JSON.parse(inputValue);
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/room/${encodeURIComponent(code)}/join`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function setRoomTags(inputValue: string) {
	if (!inputValue)
		return console.warn(
			"setRoomTags requires a body (JSON) or comma-separated tags",
		);
	let body: any;
	try {
		body = JSON.parse(inputValue);
	} catch {
		body = { tags: inputValue.split(",").map((s) => s.trim()) };
	}
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/room/tags`,
			getFetchOptions("PUT", body),
		);
		const data = await res.json();
		console.log("Set Room Tags:", data);
	} catch (err) {
		console.error("Error setting room tags:", err);
	}
}
...
async function removeRoomLink(inputValue: string) {
	if (!inputValue)
		return console.warn(
			'removeRoomLink requires "roomId|linkId" or roomId',
		);
	if (inputValue.includes("|")) {
		const [roomId, linkId] = inputValue.split("|").map((s) => s.trim());
		try {
			const res = await fetch(
				`${formbarUrl}/api/v1/room/${encodeURIComponent(roomId)}/links/${encodeURIComponent(linkId)}`,
				getFetchOptions("DELETE"),
			);
			const data = await res.json();
			console.log("Remove Room Link (by id):", data);
		} catch (err) {
			console.error("Error removing room link:", err);
		}
	} else {
		try {
			const res = await fetch(
				`${formbarUrl}/api/v1/room/${encodeURIComponent(inputValue)}/links`,
				getFetchOptions("DELETE"),
			);
			const data = await res.json();
...
async function getRoomLinks(inputValue: string) {
	...
	try {
		...
		console.error("Error getting room links:", err);
	}
}
...
async function addRoomLink(inputValue: string) {
	if (!inputValue)
		return console.warn(
			'addRoomLink requires "roomId|json" or JSON with classId',
		);
	let body: any = {};
	let roomId = "";
	if (inputValue.includes("|")) {
		...
		try {
			body = JSON.parse(inputValue);
			roomId = body.roomId || body.classId || "";
		} catch {
			return console.warn(
				"addRoomLink: provide roomId|json or valid JSON",
			);
		}
	}
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/room/${encodeURIComponent(roomId)}/links/add`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
		console.log("Add Room Link:", data);
	} catch (err) {
		console.error("Error adding room link:", err);
	}
}
...
async function updateRoomLink(inputValue: string) {
	if (!inputValue)
		return console.warn(
			'updateRoomLink requires "roomId|json" or JSON with classId',
		);
	let body: any = {};
	let roomId = "";
	if (inputValue.includes("|")) {
		...
		try {
			...
			return console.warn(
				"updateRoomLink: provide roomId|json or valid JSON",
			);
		}
	}
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/room/${encodeURIComponent(roomId)}/links`,
			getFetchOptions("PUT", body),
		);
		const data = await res.json();
		console.log("Update Room Link:", data);
	} catch (err) {
		console.error("Error updating room link:", err);
	}
}
...
async function awardDigipogs(inputValue: string) {
	if (!inputValue)
		return console.warn(
			'awardDigipogs requires body JSON or "userId|amount"',
		);
	let body: any;
	try {
		body = JSON.parse(inputValue);
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/digipogs/award`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
		console.log("Award Digipogs:", data);
	} catch (err) {
		console.error("Error awarding digipogs:", err);
	}
}
...
async function transferDigipogs(inputValue: string, bodyValue: string) {
	if (!inputValue && !bodyValue)
		return console.warn(
			'transferDigipogs requires body JSON or "toUserId|amount"',
		);
	let body: any;
	try {
		body = JSON.parse(bodyValue);
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/digipogs/transfer`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
		console.log("Transfer Digipogs:", data);
	} catch (err) {
		console.error("Error transferring digipogs:", err);
	}
}
...
async function removeIP(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/ip/${encodeURIComponent(type)}/${encodeURIComponent(id)}`,
			getFetchOptions("DELETE"),
		);
		const data = await res.json();
...
async function getIPList(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/ip/${encodeURIComponent(inputValue)}`,
			getFetchOptions(),
		);
		const data = await res.json();
		console.log("Get IP List:", data);
	} catch (err) {
		console.error("Error getting IP list:", err);
	}
}
...
async function toggleIP(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/ip/${encodeURIComponent(type)}/toggle`,
			getFetchOptions("POST"),
		);
		const data = await res.json();
...
async function updateIP(inputValue: string) {
	if (!inputValue)
		return console.warn(
			'updateIP requires "type|id|json" or JSON with type and id',
		);
	let type = "" as string;
	let id = "" as string;
	let body: any = {};
	if (inputValue.includes("|")) {
		...
		try {
			...
			return console.warn(
				"updateIP: provide type|id|json or JSON with type and id",
			);
		}
	}
...
async function getLogFile(inputValue: string) {
	...
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/logs/${encodeURIComponent(inputValue)}`,
			getFetchOptions(),
		);
...
async function oauthAuthorize() {
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/oauth/authorize`,
			getFetchOptions(),
		);
		const data = await res.json();
		console.log("OAuth Authorize:", data);
	} catch (err) {
		console.error("Error calling oauth authorize:", err);
	}
}

async function oauthRevoke(inputValue: string) {
	if (!inputValue)
		return console.warn("oauthRevoke requires body (token or json)");
	let body: any;
	try {
		body = JSON.parse(inputValue);
	} catch {
		body = { token: inputValue };
	}
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/oauth/revoke`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
...
async function oauthToken(inputValue: string) {
	...
	try {
		...
		return console.warn("oauthToken requires JSON body");
	}
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/oauth/token`,
			getFetchOptions("POST", body),
		);
		const data = await res.json();
		console.log("OAuth Token:", data);
	} catch (err) {
		console.error("Error requesting oauth token:", err);
	}
}

async function getUserTransactions(inputValue: string) {
	if (!inputValue)
		return console.warn("getUserTransactions requires a userId");
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/${encodeURIComponent(inputValue)}/transactions`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getUserPools() {
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/user/pools`,
			getFetchOptions(),
		);
		const data = await res.json();
		console.log("Get User Pools:", data);
	} catch (err) {
		console.error("Error getting user pools:", err);
	}
}
...
async function getNotifications() {
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/notifications`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function getNotifByID(notifId: string) {
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/notifications/${encodeURIComponent(notifId)}`,
			getFetchOptions(),
		);
		const data = await res.json();
...
async function markNotifAsRead(notifId: string) {
	try {
		const res = await fetch(
			`${formbarUrl}/api/v1/notifications/${encodeURIComponent(notifId)}/mark-read`,
			getFetchOptions("POST", { read: true }),
		);
		const data = await res.json();
